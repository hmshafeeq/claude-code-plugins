---
name: compound-docs
description: This skill should be used when capturing solved problems as categorized documentation with YAML frontmatter for fast lookup. It provides a structured process for documenting solutions immediately after confirmation.
allowed-tools:
...
**Organization:** Single-file architecture - each problem documented as one markdown file in its symptom category directory (e.g., `docs/solutions/performance-issues/slow-query-fix.md`). Files use YAML frontmatter for metadata and searchability.
...
- **Module/Component name**: Which part of the system had the problem
...
**BLOCKING REQUIREMENT:** If critical context is missing (module name, exact error, or resolution steps), ask user and WAIT for response before proceeding:

```
I need a few details to document this properly:

1. Which module/component had this issue?
2. What was the exact error message or symptom?
3. What stage of development were you in?
...
**Examples:**
- `missing-include-user-service-20251110.md`
- `parameter-not-saving-api-handler-20251110.md`
- `timeout-error-payment-processor-20251110.md`

### Step 5: Validate YAML Schema

**CRITICAL:** All docs require validated YAML frontmatter with enum validation.

Validate against [schema.yaml](./schema.yaml) and classify the problem against the enum values defined in [yaml-schema.md](./references/yaml-schema.md). Ensure all required fields are present and match allowed values exactly.
...
**Do NOT proceed to Step 6 until YAML frontmatter passes all validation rules.**
...
When user selects Option 2 (Add to Required Reading), use the template from [critical-pattern-template.md](./assets/critical-pattern-template.md) to structure the pattern entry.
...
- Confirm: "Added to [skill-name] skill"
...
2. **Gather context:**
   - Module: User Service
   - Symptom: User list taking >5 seconds, N+1 query when loading user profiles
...
4. **Generate filename:** `n-plus-one-user-list-UserService-20251110.md`
5. **Validate YAML:**
   ```yaml
   module: User Service
   date: 2025-11-10
...
6. **Create documentation:**
   - `docs/solutions/performance-issues/n-plus-one-user-list-UserService-20251110.md`
